...
int text_init() {
  ...
  int p=0; for (;;p++) {
    ...
    if (!tid) break;
    if (tid>EGG_TID_string) break;
    if (tid<EGG_TID_string) continue;
    if (text.langc&&(text.langv[text.langc-1]==qual)) continue;
    uint8_t hi=qual>>8,lo=qual; // Don't record qualifiers that aren't 639 codes.
...
int text_get_string(const char **v,int stringid) {
  ...
  if (!tmpv) { *v=""; return 0; }
  for (;;) {
    tmpc=egg_res_get(tmpv,tmpa,EGG_TID_string,text.lang,stringid);
    if (tmpc<0) tmpc=0;
    if (tmpc<tmpa) {
      tmpv[tmpc]=0;
      break;
...
static int text_require_metadata() {
  ...
  for (;;) {
    c=egg_res_get(text.metav,a,EGG_TID_metadata,0,1);
    if (c<0) c=0;
    if (c<=a) {
      text.metac=c;
      return 0;
